...
./build/optimize_cpu \
  ...
  --gen.af-device=0 \
  --gen.output-csv=./log/optimize-${TOPO}-500-gallancchio-mg${TAU}.csv \
  --opt.cmaes-fplot=./log/optimize-${TOPO}-500-gallancchio-mg${TAU}-run@RUN@.dat \
  2>&1 | tee ./log/optimize-${TOPO}-500-gallancchio-mg${TAU}.log
