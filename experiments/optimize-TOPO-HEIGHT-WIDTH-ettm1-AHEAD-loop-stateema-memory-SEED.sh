...
outdir="./log/optimize-${TOPO}-${HEIGHT}-${WIDTH}-ettm1-ahead${AHEAD}-loop-stateema-memory-seed${SEED}/"
mkdir -p "${outdir}"
./build/optimize_cuda \
  --gen.net-type=lcnn \
  ...
  --opt.include-params=lcnn.mu-memory lcnn.mu-state-ema-alpha lcnn.mu-state-ema-w \
