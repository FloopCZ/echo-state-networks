...
./build/optimize_cuda \
  ...
  --gen.af-device=0 \
  --gen.output-csv=./log/optimize-${TOPO}-nofb-200-dale-narma30.csv \
  --opt.cmaes-fplot=./log/optimize-${TOPO}-nofb-200-dale-narma30-run@RUN@.dat \
  2>&1 | tee ./log/optimize-${TOPO}-nofb-200-dale-narma30.log
