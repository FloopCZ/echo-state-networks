  ...
  prng_t& prng)
{
    ...
    if (topo_params.contains("sparse")) {
        ...
        cfg.reservoir_w = sigma_res * (af::randu(res_dims, DType, af_prng) * 2 - 1);
        // normalize the weights such that the mean for each kernel is equal to mu_res
        af::array mu_kernel = af::mean(
          af::moddims(cfg.reservoir_w, state_height, state_width, kernel_height * kernel_width), 2);
        cfg.reservoir_w += af::tile(mu_res - mu_kernel, 1, 1, kernel_height, kernel_width);
        // make the reservoir sparse by the given coefficient
        cfg.reservoir_w *= af::randu({cfg.reservoir_w.dims()}, DType, af_prng) >= sparsity;
    ...
    {
        af::array curr_mu = af::mean(af::flat(cfg.reservoir_b));
        cfg.reservoir_b += af::tile(mu_b - curr_mu, state_height, state_width);
    }
    ...
    if (n_input_neurons == state_height * state_width) {
        ...
        for (long i = 0; i < n_ins; ++i) {
            ...
            af::array curr_mu = af::mean(af::flat(cfg.input_w(af::span, af::span, i)));
            cfg.input_w(af::span, af::span, i) +=
              af::tile(mu_in_weight.at(i) - curr_mu, state_height, state_width);
        }
        ...
        for (long i = 0; i < n_outs; ++i) {
            ...
            af::array curr_mu = af::mean(af::flat(cfg.feedback_w(af::span, af::span, i)));
            cfg.feedback_w(af::span, af::span, i) +=
              af::tile(mu_fb_weight.at(i) - curr_mu, state_height, state_width);
        }
    } else {
